...
enum ThreeERegistryError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidAppsCollection:
            return "الحقل apps داخل registry.json ليس قائمة صالحة، لذلك لم يتم تعديله."
        }
    }
}
    ...
    static func registerLiDARApp(in threeERootURL: URL) throws {
        ...
        coordinator.coordinate(writingItemAt: systemURL, options: [], error: &coordinationError) { coordinatedSystemURL in
            do {
                ...
                if let index = apps.firstIndex(where: { ($0["appKey"] as? String) == ThreeEStorageConstants.appKey }) {
