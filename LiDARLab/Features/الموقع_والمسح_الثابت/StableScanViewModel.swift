...
final class StableScanViewModel: ObservableObject {
    ...
    @Published private(set) var lastAcknowledgedFrameID: UInt64 = 0
    ...
    var lightText: String {
        guard let value = preview.ambientIntensity else { return "إضاءة —" }
        return String(format: "إضاءة %.0f", value)
    }

    var isLowLight: Bool {
        guard let value = preview.ambientIntensity else { return false }
        return value < 250
    }
    ...
    private func configureCallbacks() {
        ...
        network.onState = { [weak self] connected, message in
            Task { @MainActor in
                ...
                if !connected { self?.networkReady = false }
                self?.networkStatus = message
            }
        }
        network.onReceiverReady = { [weak self] ready, message in
            Task { @MainActor in
                self?.networkReady = ready
                self?.networkStatus = message
            }
        }
        network.onAcknowledgement = { [weak self] frameID, status in
            Task { @MainActor in
                self?.lastAcknowledgedFrameID = max(self?.lastAcknowledgedFrameID ?? 0, frameID)
                self?.networkReady = true
                self?.networkStatus = "Windows أكد \(status) عند Frame \(frameID)"
            }
        }
