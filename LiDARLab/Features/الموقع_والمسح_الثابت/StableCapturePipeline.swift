...
final class StableCapturePipeline: @unchecked Sendable {
    ...
    private let motionMonitor = StableMotionMonitor()
    ...
    private var latestCameraCoverageSamples: [StableCameraCoverageSample] = []
    private var latestCameraImageWidth = 0
    private var latestCameraImageHeight = 0
    private var latestAmbientIntensity: CGFloat?
    private var latestMotionSnapshot = StableMotionSnapshot.unavailable
    ...
    func start(
        ...
        previewQueue.async { [weak self] in
            ...
            self.latestCameraCoverageSamples.removeAll(keepingCapacity: true)
            self.latestCameraImageWidth = 0
            self.latestCameraImageHeight = 0
            self.latestAmbientIntensity = nil
        }
    ...
    func ingest(frame: ARFrame) {
        ...
        let motionSnapshot = motionMonitor.snapshot()
        let cameraImageWidth = Int(frame.camera.imageResolution.width)
        let cameraImageHeight = Int(frame.camera.imageResolution.height)
    ...
    func resetPreview() {
        previewQueue.async { [weak self] in
            ...
            self.lastPreviewPose = nil
            self.forceNewSegment = true
            self.latestCameraCoverageSamples.removeAll()
            self.latestCameraImageWidth = 0
            self.latestCameraImageHeight = 0
            self.latestAmbientIntensity = nil
    ...
    private func processDepth(_ input: StableDepthInput, sessionID: UInt64, sendNetwork: Bool) {
        ...
        let cameraSamples = cameraCoverageSamples(from: input)
        previewQueue.async { [weak self] in
            guard let self else { return }
            self.latestCameraCoverageSamples = cameraSamples
            self.latestCameraImageWidth = Int(input.imageResolution.width)
            self.latestCameraImageHeight = Int(input.imageResolution.height)
            if !endpoints.isEmpty {
                self.rasterizeCoverage(from: input.pose, endpoints: endpoints, cellSize: input.settings.previewCellSize)
            }
            self.publishPreview(currentPose: input.pose)
        }
    }

    private func enqueuePreviewPose(
        _ pose: StablePoseSample,
        settings: StableCaptureSettings,
        ambientIntensity: CGFloat?,
        cameraImageWidth: Int,
        cameraImageHeight: Int,
        motionSnapshot: StableMotionSnapshot
    ) {
        previewQueue.async { [weak self] in
            guard let self else { return }
            self.latestAmbientIntensity = ambientIntensity
            self.latestCameraImageWidth = cameraImageWidth
            self.latestCameraImageHeight = cameraImageHeight
            self.latestMotionSnapshot = motionSnapshot
    ...
    private func publishStatusOnly(
        _ pose: StablePoseSample,
        ambientIntensity: CGFloat?,
        cameraImageWidth: Int,
        cameraImageHeight: Int
    ...
    private func appendPreviewPose(_ pose: StablePoseSample) {
        ...
        if let previous = lastPreviewPose, previous.trackingState == 2 {
            ...
            if dt <= 0 || dt > 0.35 {
                ...
                if latestMotionSnapshot.indicatesNearStillness {
                    breakReason = "قفزة Pose غير مدعومة بحركة IMU \(String(format: "%.2f", distance)) م"
                } else {
    ...
    private func publishPreview(currentPose: StablePoseSample?) {
        ...
        let snapshot = StablePreviewSnapshot(
            ...
            cameraCoverageSamples: latestCameraCoverageSamples,
            cameraImageWidth: latestCameraImageWidth,
            cameraImageHeight: latestCameraImageHeight,
    ...
    private func cameraCoverageSamples(from input: StableDepthInput) -> [StableCameraCoverageSample] {
        let depthMap = input.depthMap
        let width = CVPixelBufferGetWidth(depthMap)
        let height = CVPixelBufferGetHeight(depthMap)
        ...
        let columns = 18
        let rows = 12
        var samples: [StableCameraCoverageSample] = []
        samples.reserveCapacity(columns * rows)

        for rowIndex in 0..<rows {
            let v = (Float(rowIndex) + 0.5) / Float(rows)
            let sourceY = min(max(Int(v * Float(height)), 0), height - 1)
            let depthRow = depthBase
                .advanced(by: sourceY * depthBytesPerRow)
                .assumingMemoryBound(to: Float32.self)

            for columnIndex in 0..<columns {
                let u = (Float(columnIndex) + 0.5) / Float(columns)
                let sourceX = min(max(Int(u * Float(width)), 0), width - 1)
                let depthMeters = depthRow[sourceX]
                guard depthMeters.isFinite,
                      depthMeters >= input.settings.previewMinimumDepth,
                      depthMeters <= input.settings.previewMaximumDepth else { continue }

                var confidence: UInt8 = 1
                if let confidenceBase, confidenceWidth > 0, confidenceHeight > 0 {
                    let confidenceX = min(Int(Float(sourceX) * Float(confidenceWidth) / Float(width)), confidenceWidth - 1)
                    let confidenceY = min(Int(Float(sourceY) * Float(confidenceHeight) / Float(height)), confidenceHeight - 1)
                    let confidenceRow = confidenceBase
                        .advanced(by: confidenceY * confidenceBytesPerRow)
                        .assumingMemoryBound(to: UInt8.self)
                    confidence = confidenceRow[confidenceX]
                }
                samples.append(StableCameraCoverageSample(
                    u: u,
                    v: v,
